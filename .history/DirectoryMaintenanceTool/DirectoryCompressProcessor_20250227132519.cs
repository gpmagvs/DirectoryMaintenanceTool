...
public class DirectoryCompressProcessor : IDirectoryProcessor
{
    ...
    private void ProcessSingleDirectory(string directoryPath)
    {
        try
        {
            ...
            {
                Logger.Info($"[保留] {dirInfo.Name} 建立時間較新 ({dirInfo.CreationTime:yyyy-MM-dd HH:mm:ss})");
                return;
            }

            if (dirInfo.CreationTime.Date == DateTime.Now.Date)
            {
                Logger.Info($"[保留] {dirInfo.Name} 建立時間為今日 ({dirInfo.CreationTime:yyyy-MM-dd HH:mm:ss})");
                return;
            }
            ...
            {
                Logger.Info($"[保留] {dirInfo.Name} 已經有壓縮檔");
                return;
            }
            ...
            try
            {
                ...
                Logger.Info($"[壓縮] {dirInfo.Name} 建立時間: {dirInfo.CreationTime:yyyy-MM-dd HH:mm:ss} 原始大小: {FormatFileSize(dirSize)}");
                ...
                if (ValidateZipFile(zipPath))
                {
                    ...
                    Logger.Info($"壓縮大小: {FormatFileSize(zipInfo.Length)}, 壓縮比例: {((1 - (double)zipInfo.Length / dirSize) * 100):F2}%");
