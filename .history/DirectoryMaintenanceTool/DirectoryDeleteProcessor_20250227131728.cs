    ...
    public class DirectoryDeleteProcessor : IDirectoryProcessor
    {
        ...
        public void ProcessDirectories()
        {
            try
            {
                ...
                Logger.Info($"找到 {directories.Length} 個子目錄需要處理");
                Logger.Info("開始處理...\n");
                ...
                // 輸出處理摘要
                Logger.Info("\n處理完成摘要:");
                Logger.Info($"總共掃描: {_processedCount} 個目錄");
                Logger.Info($"已刪除: {_deletedCount} 個目錄");
                Logger.Info($"保留: {_processedCount - _deletedCount} 個目錄");
            }
            catch (Exception ex)
            {
